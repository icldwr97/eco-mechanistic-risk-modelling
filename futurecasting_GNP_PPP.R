...
#GNP,PPP
mortsX<-read.csv("C:\\Users\\kjones\\Downloads\\ny.gdp.pcap.pp.cd_Indicator_en_csv_v2\\ny.gdp.pcap.pp.cd_Indicator_en_csv_v2b.csv")


plot(tt$year,1/(tt$mort),xlim=c(a,max(tt3$year)),ylim=c(0,max(1/60000)),pch=20)
...
a<-as.numeric(gsub("X","",rownames(tt)[1]))###earliest date
b<-as.numeric(gsub("X","",rownames(tt)[nrow(tt)]))###latest date
c<-2070
...
#fit decay curve
mod <- nls(1/mort ~   exp(b-a * year), data = tt, start = list(a = 0, b=0))
summary(mod)

# add fitted curve
tt3<-data.frame(year=a:c,mort=NA)
lines(a:c, predict(mod, tt3),col=rainbow(50)[sample(1:50,1)])

tt2<-tt[1:2,]
tt2$year<-c(2050,2070)
tt2$mort<-NA
res1<-data.frame(country=mortsX$Country.Name[i],years=c(2050,2070),est=1/predict(mod,tt2))
...
write.csv(res2,file="C:\\Users\\kjones\\Dropbox\\ebola\\GNP_PPP_prediction.csv")

ests<-read.csv(file="C:\\Users\\kjones\\Dropbox\\ebola\\GNP_PPP__prediction.csv",stringsAsFactors=F)
